...
$listagem = $conexao->prepare("SELECT * FROM loja WHERE status = 2");
$listagem->execute();
?>
        ...
        <div class="row">
            <div class="col dadosProduto">
                ...
                <?php while ($lista = $listagem->fetch(PDO::FETCH_ASSOC)) : ?>
                    ...
                    <a href="../backend/removeProtudo.php?id=<?php echo $lista['id'] ?>"><button type="button" class="btn btn-outline-danger">Remover do carrinho</button></a>
                    <?php $sum += $lista['preco']; ?>
                    <hr>
                <?php endwhile ?>

                <p style="margin-top: 10px;"><b>Total:</b> R$: <?php echo $sum ?>,00</p>
